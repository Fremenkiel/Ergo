...
pub const TlsContext = struct {
    ...
    pub fn connect(self: *TlsContext, stream: std.net.Stream) !void {
        ssl.SSL_set_fd(self.ssl, @as(c_int, @intCast(stream.handle)));
    ...
    pub fn read(self: *TlsContext, buffer: []u8) !usize {
        const result = ssl.SSL_read(self.ssl, buffer.ptr, @as(c_int, @intCast(buffer.len)));
        if (result <= 0) {
    ...
    pub fn write(self: *TlsContext, buffer: []const u8) !usize {
        const result = ssl.SSL_write(self.ssl, buffer.ptr, @as(c_int, @intCast(buffer.len)));
        if (result <= 0) {
            const err = ssl.SSL_get_error(self.ssl, result);
            return switch (err) {
                ssl.SSL_ERROR_WANT_READ => error.WouldBlock,
